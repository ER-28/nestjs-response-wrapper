...
import { INestApplication, Valve, HttpStatus } from '@nestjs/common';
...
import { Controller, Get, Param, Res, HttpStatus as HttpStatusCode } from '@nestjs/common';
...
describe('ResponseWrapper Integration', () => {
  ...
  beforeAll(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        { provide: 'RESPONSE_WRAPPER_OPTIONS', useValue: { version: '1.0.0', debug: true, excludeRoutes: [] } }
      ],
    }).compile();

    app = moduleFixture.createNestApplication();
    app.useGlobalInterceptors(new ResponseInterceptor(moduleFixture.get('RESPONSE_WRAPPER_OPTIONS'), new (require('@nestjs/core').Reflector)()));
    app.useGlobalFilters(new ResponseExceptionFilter(moduleFixture.get('RESPONSE_WRAPPER_OPTIONS')));
    await app.init();
  });
